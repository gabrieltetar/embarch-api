...
//!   example: `es_nrf54dk` declares `nrf54l05`/`nrf54l10`/an `nrf54l15`
...
struct BoardSection {
    ...
    /// against the real healthband repo's `board.yml` files (`roadrunner`,
    /// `nff_dev`, `es_nrf54dk`, `nff_demo`), all four of which nest it here.
...
/// nesting level above it — confirmed against the real repo: `es_nrf54dk`'s
...
/// (`es_nrf54dk`'s `nrf54l05`) — handled the same way an empty `variants`
...
/// A SoC with no declared `variants` (e.g. `es_nrf54dk`'s `nrf54l05`) gets
...
///
/// A variant literally named `default` (e.g. `nff_dev`, `nff_demo`) is
/// treated the same way — confirmed against the real repo, not assumed: its
/// name never appears in any board filename (`nff_dev_nrf54l15_cpuapp.dts`,
...
/// building `nff_dev/nrf54l15/cpuapp/default` instead of the real
/// `nff_dev/nrf54l15/cpuapp`).
///
...
fn push_targets_for_soc(out: &mut Vec<Target>, board: &BoardDef, soc: &SocSection, apps: &[String]) {
    ...
    // is the check that excludes `es_nrf54dk`'s `nrf54l05`/`nrf54l10`/
...
/// real `es_nrf54dk` finding this exists to catch.
...
/// `max86178-evk`, `max86178-evt-3led`, `max86178-evt-5led`, `max_signal`)
...
"#,
    ...
    /// Regression test using the real healthband repo's actual
    /// `boards/nordic/nff_dev/board.yml` content verbatim (checked directly
    ...
    #[test]
    fn real_nff_dev_board_yml_resolves_every_real_revision() {
        let dir = tempfile_dir();
        let board_dir = dir.path().join("boards/nordic/nff_dev");
        fs::create_dir_all(&board_dir).unwrap();
        fs::write(
...
board:
  name: nff_dev
  full_name: EightSleep NFF Development Board
  vendor: nordic
  socs:
    - name: nrf54l15
      variants:
...
"#,
        ...
        .unwrap();
        fs::write(board_dir.join("nff_dev_nrf54l15_cpuapp.dts"), "").unwrap();
        for rev in ["6", "7"] {
            fs::write(board_dir.join(format!("nff_dev_nrf54l15_cpuapp_{rev}.overlay")), "").unwrap();
        }
        let app_dir = dir.path().join("app/healthband");
        fs::create_dir_all(&app_dir).unwrap();
        fs::write(app_dir.join("CMakeLists.txt"), "").unwrap();
        ...
        assert_eq!(picked.board_qualifier(), "nff_dev@7/nrf54l15/cpuapp");
    }
    ...
    fn select_narrows_a_singleton_variant_without_it_being_typed() {
        // A SoC with no variants declared at all (real example: es_nrf54dk's
...
"#,
    ...
    fn soc_with_no_dts_backing_at_all_yields_no_targets() {
        // Real example this guards: es_nrf54dk's board.yml declares
        // nrf54l05 with no variants at all, but the real repo has no
        // es_nrf54dk_nrf54l05.dts file — must yield zero targets for that
...
"#,
    ...
    fn named_variant_with_no_dts_is_excluded_but_sibling_with_dts_survives() {
        // Real example this guards: es_nrf54dk's nrf54l15 SoC declares two
...
"#,
    ...
    fn build_command_places_extra_args_right_after_the_build_subcommand() {
        ...
        // `west build -p always -b nff_dev@7/nrf54l15/cpuapp -S ble-shell -d
        // <dir> <app>`.
        assert_eq!(
            cmd,
            vec![
